...
def main():
    extracted_cfg = 0
    file_paths = list_files("new_contracts_part/")
    # Using multiprocessing Pool to parallelize execution
    with Pool() as pool:
        results = pool.map(run_combined_attack, file_paths)
        extracted_cfg = sum(results)
